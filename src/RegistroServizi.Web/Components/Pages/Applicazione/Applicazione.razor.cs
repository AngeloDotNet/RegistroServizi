...
public partial class Applicazione : IDisposable
{
    ...
    protected override async Task OnInitializedAsync()
    {
        try
        {
            var applicazioni = await ApplicazioneService.GetAllApplicazioniAsync().ConfigureAwait(false);
            ...
            await InvokeAsync(() =>
            {
                elements = applicazioni?.ToList() ?? [];
            });
        }
        catch (Exception ex)
        {
            Logger.LogError(ex, "Errore durante il caricamento delle applicazioni. Message: {Message}", ex.Message);
            await InvokeAsync(() => Snackbar.Add("Errore durante il caricamento delle applicazioni.", Severity.Error));
        }
        finally
        {
    ...
    private void StartedEditingItem(ApplicazioneDto item) => isEditing = true;
    private void CanceledEditingItem(ApplicazioneDto item) => isEditing = false;
    ...
    private async Task<DataGridEditFormAction> CommittedItemChangesAsync(ApplicazioneDto item)
    {
        ...
        try
        {
            await ApplicazioneService.UpdateApplicazioneAsync(updatedItem).ConfigureAwait(false);
            ...
            await InvokeAsync(() => Snackbar.Add("L'applicazione è stata aggiornata con successo.", Severity.Success));
    ...
    private void ValidateVersione(string value)
    {
        ...
        if (!ApplicazioneMapper.VersionRegex.IsMatch(value))
        {
    ...
    private bool ValidateApplicazione(ApplicazioneDto item)
    {
        ...
        if (string.IsNullOrWhiteSpace(item.Versione) || !ApplicazioneMapper.VersionRegex.IsMatch(item.Versione))
        {
